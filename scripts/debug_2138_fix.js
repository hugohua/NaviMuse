...
// Apply regex 2 (The generic one)
const prevLen = content.length;
content = content.replace(/}\s*,\s*"embedding_tags"\s*:/g, ',"embedding_tags":');
console.log('Regex 2 applied. Change in length:', content.length - prevLen);
...
try {
    ...
    if (e.message.includes('position')) {
        ...
        if (match) {
            ...
            console.log('Indices:');
            for (let k = start; k < end; k++) console.log(`${k}: ${content[k]}`);
        }
    }
}
